...
Identity 2,1 expõe pontos de extremidade com a `Identity` área. Por exemplo, a tabela a seguir mostra exemplos de Identity pontos de extremidade que mudam de 2,0 para 2,1:
...
### <a name="update-no-locidentity-to-version-21"></a>Atualizar Identity para a versão 2,1
...
* Exclua seu Identity código 2,0 existente [e Identity Scaffold](xref:security/authentication/scaffold-identity) -o em seu projeto. Seu projeto usará a [ASP.NET Core Identity](xref:security/authentication/identity) [ Razor biblioteca de classes](xref:razor-pages/ui-class). Você pode gerar código e interface do usuário para qualquer Identity código da interface do usuário que você modificou. Aplique as alterações de código ao código da interface do usuário do com Scaffold recentemente.
...
### <a name="replace-no-locidentity-20-ui-with-the-no-locidentity-21-no-locrazor-class-library"></a>Substitua Identity a interface do usuário 2,0 pela Identity Razor biblioteca de classes 2,1

Esta seção descreve as etapas para substituir o código gerado pelo modelo ASP.NET Core 2,0 Identity pela biblioteca de [ASP.NET Core Identity](xref:security/authentication/identity) [ Razor classes](xref:razor-pages/ui-class). As etapas a seguir são para um Razor projeto de páginas, mas a abordagem para um projeto MVC é semelhante.
...
### <a name="update-after-scaffolding-no-locidentity"></a>Atualizar após scaffolding Identity

* Exclua a Identity `IdentityDbContext` classe derivada gerada scaffolder na pasta *areas/ Identity /Data/* .
* Excluir *áreas/ Identity / Identity HostingStartup.cs*.
...
## <a name="changes-to-no-locrazor-pages-projects-no-locrazor-files"></a>Alterações em Razor páginas arquivos de projetos Razor
...
## <a name="changes-to-mvc-projects-no-locrazor-files"></a>Alterações nos arquivos de projetos do MVC Razor
...
* [Configuração de transporte Libuv](xref:fundamentals/servers/kestrel#libuv-transport-configuration)
