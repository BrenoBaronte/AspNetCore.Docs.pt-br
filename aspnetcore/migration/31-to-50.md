...
## <a name="changes-to-no-locblazor-app-routing-logic-in-501"></a>Alterações Blazor na lógica de roteamento de aplicativo no 5.0.1
...
>
> A capacidade de definir `PreferExactMatches` como `@false` ou deixá-la desdefinida, que usa como padrão a opção `false` em C#, *é fornecida somente para compatibilidade com versões anteriores*.
>
...
## <a name="update-a-no-locblazor-progressive-web-application-pwa"></a>Atualizar um Blazor aplicativo Web progressivo (PWA)
