...
* Usa o sistema de [model binding](xref:mvc/models/model-binding) para converter valores de formulário de cadeias de caracteres para os tipos no modelo `Student`. Por exemplo, `EnrollmentDate` é convertido em `DateTime` .
...
Seja qual for o valor que o invasor especificou para o campo de formulário `Secret`, ele será atualizado no banco de dados. A imagem a seguir mostra a ferramenta Fiddler adicionando o `Secret` campo, com o valor "overpost", aos valores de formulário postados.
...
* `Modified`: alguns ou todos os valores de propriedade da entidade foram modificados. O `SaveChanges` método emite uma `UPDATE` instrução.

* `Deleted`: a entidade foi marcada para exclusão. O `SaveChanges` método emite uma `DELETE` instrução.

* `Detached`: A entidade não está sendo rastreada pelo contexto do banco de dados.

Em um aplicativo da área de trabalho, em geral, as alterações de estado são definidas automaticamente. Uma entidade é lida, as alterações são feitas e o estado da entidade é alterado automaticamente para `Modified`. Chamar `SaveChanges` gera uma `UPDATE` instrução SQL que atualiza apenas as propriedades alteradas.
...
Nesta seção, uma mensagem de erro personalizada é implementada quando a chamada para `SaveChanges` falha.

Substitua o código em *Pages/Students/Delete.cshtml.cs* pelo código a seguir. As alterações são realçadas:

[!code-csharp[Main](intro/samples/cu50/Pages/Students/Delete.cshtml.cs?name=snippet_All&highlight=12-14,22,30-33,45-99)]

O código anterior adiciona o parâmetro `saveChangesError` opcional à assinatura do método `OnGetAsync`. `saveChangesError` indica se o método foi chamado após uma falha ao excluir o objeto de aluno. A operação de exclusão pode falhar devido a problemas de rede temporários. Erros de rede transitórios são mais prováveis quando o banco de dados está na nuvem. O `saveChangesError` parâmetro é `false` quando a página de exclusão `OnGetAsync` é chamada da interface do usuário. Quando `OnGetAsync` é chamado por `OnPostAsync` porque a operação de exclusão falhou, o `saveChangesError` parâmetro é `true` .

O método `OnPostAsync` recupera a entidade selecionada e, em seguida, chama o método [Remove](/dotnet/api/microsoft.entityframeworkcore.dbcontext.remove#Microsoft_EntityFrameworkCore_DbContext_Remove_System_Object_) para definir o status da entidade como `Deleted`. Quando `SaveChanges` é chamado, um `DELETE` comando SQL é gerado. Se `Remove` falhar:
...
O código com scaffold das páginas Alunos não inclui dados de registro. Nesta seção, os registros são adicionados à página de detalhes.

### <a name="read-enrollments"></a>Ler inscrições

Para exibir os dados de registro de um aluno na página, os dados do registro precisam ser lidos. O código com scaffold em *Pages/estudantes/details.cshtml.cs* lê somente os dados do Aluno, sem os dados de Registro:
...
* `FirstOrDefaultAsync` foi substituído por [FindAsync](/dotnet/api/microsoft.entityframeworkcore.dbset-1.findasync). Quando os dados relacionados incluídos não são necessários, o `FindAsync` é mais eficiente.
* `OnPostAsync` tem um parâmetro `id`.
* O aluno atual é buscado do banco de dados, em vez de criar um aluno vazio.
