...
Execute o seguinte comando da CLI do .NET:

```dotnetcli
dotnet tool install --global dotnet-aspnet-codegenerator
```

O comando anterior adiciona a [ferramenta ASPNET-CodeGenerator scaffolding](xref:fundamentals/tools/dotnet-aspnet-codegenerator).
...
1. No PMC, insira os seguintes comandos:
   ...
   O `Update-Database` comando executa o `Up` método no arquivo *Migrations/{time-Stamp} _InitialCreate. cs* , que cria o banco de dados.
