...
1. Selecione a ramificação padrão (*mestre*) do **Branch padrão para a lista suspensa de compilações manuais e agendadas** . Clique no botão **Continuar**. A página seleção de modelo é exibida.
...
1. Envie por push a alteração no Branch padrão (*mestre*) para o remoto de *origem* do seu repositório do github. No comando a seguir, substitua o espaço reservado `{BRANCH}` pelo Branch padrão (use `master` ):

    ```console
    git push origin {BRANCH}
    ```

    A confirmação aparece na ramificação padrão do repositório GitHub (*mestre*):
